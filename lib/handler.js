  ...
  var assetCache = server.cache({
    ...
    generateFunc: function(id, next) {
      ...
      processExtension(extension, id.filepath, function(err, contents) {
        if (err) {
          server.log(['assets', 'error'], { message: 'error processing file', extension: id.fileExtension, filepath: id.filepath, error: err });
          return next(err);
        }
        server.log(['assets', 'info'], { message: 'processed file', extension: id.fileExtension, filepath: id.filepath });
        next(null, contents);
